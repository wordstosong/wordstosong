...
$(window).scroll(function(){
    $(".top-section span").css("opacity", 1 - $(window).scrollTop() / 500);
    $(".top-section p").css("opacity", 1 - $(window).scrollTop() / 500);
    $(".top-section ul").css("opacity", 1 - $(window).scrollTop() / 500);
    $(".process-hero-content").css("opacity", 1 - $(window).scrollTop() / 500);
  });
...
$(function(){
  $("#headshot").on({
   mouseenter: function(){
    $(this).attr('src', '/images/icons/chs.png');
  // },
  // mouseleave: function(){
  //   $(this).attr('src','/images/icons/chs.png');
  }
...
function particle(){
  this.color = 'rgba(255, 183, 178,'+ Math.random()+')';
...
//MAGNIFIC IMAGE POPUP (WITH CAPTION AND ZOOM)
